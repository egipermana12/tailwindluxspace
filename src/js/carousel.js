...
const carouseContainer = carouselId?.getElementsByClassName("container")[0];

function carouselCalculateOffset() {
	const carouselOffset = carouseContainer.getBoundingClientRect().left;
	console.log(carouselOffset);
	carouselItems.style.paddingLeft = `${carouselOffset - 16 }px`;
	carouselItems.style.paddingLRight = `${carouselOffset - 16 }px`;
}
